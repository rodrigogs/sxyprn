...
| Post (video) | `/post/<hex12>.html` | hex id, e.g. `/post/6ab1a9bec8445.html`; bare `/post/<hex12>` also serves (microdata contentUrl) |
...
- `.vid_container[data-hvp]` with:
  ...
  - `span.duration_small` → `M:SS`/`MM:SS` duration text; inline style border
    color `#6aaf4c` (green) encodes quality bucket; `title="s<server>->c<chunk>"`.
  - `span.shd_small` → "HD" badge, monospace `#ef9639`,
    `title="bitrate:<kbps>|<quality-tier>"`.
  - `span.post_el_small_subcat` → subcategory chip.
  - `.emoji_zone[data-emoji]` → reaction counters (JSON `[{"i":<id>,"v":<count>}]`).
- `.post_control` → `a.post_time[href="/post/<hex12>.html"]` with
  `<span>` relative date ("Yesterday", "3 days ago") + `<strong>·</strong> <N> views`,
  `.vid_like_blog` like count, `.tm_orgasmic_hl` "orgasmic" count,
...
- `div.next_page` = "Next Page >" link (keep `sm` when on a tag page).
...
## Post detail page (`/post/<hex12>.html`)

- `<span class="vidsnfo" data-vnfo='{"<pid>":"<obfuscated path>"}'>` — the
  stream source (see below).
- `<video id="player_el" data-postid data-mgfs src poster>` — `src` is EMPTY in
  HTML; `getvsrc()` in `main2.js` fills it client-side.
- Schema.org microdata (VideoObject): `name`, `description`, `thumbnailUrl`,
  `uploadDate` (ISO 8601 with offset), `duration` (ISO 8601 `PT26M35S`),
  `contentUrl` (the post URL; the JS rewrites it to the resolved stream).
...
- Related wall below: the same `post_el_small` cards (a parser must scope to
  the main post container, NOT walk all cards).
...
resolved:  https://sxyprn.com/cdn8/NTgtc3h5cHJuLmNvbS03MQ../c5/a1g.../.../1790107090/.../....vid
```

Note the timestamp segment CHANGES: `tmp[5] = 1790107219 - (55 + 54)`.
